...
- **CES — the constraint-example-spec approach** this project uses. The executable spec _is_
...
- **RM — a traditional requirements-management tool** (Jira + Xray, Zephyr, DOORS, Polarion).
  ...
  The automated test is a _further_ record, mapped back to the test-case record by a sync plugin.
...
- **CES**: each fact has exactly one home. A product's expected price is a constant in a
...
- **RM**: the same fact is restated in the requirement record, again in the test-case steps,
  again in the automated test, and the matrix records that they _should_ agree. Four copies plus
...
- **RM — curated.** Traceability is a maintained artefact: a matrix of requirement IDs ↔
  ...
  requirement changes but the matrix does not, coverage _looks_ complete while a link is stale —
...
- **CES — structural.** Traceability is a side effect of how the code is wired. A boundary
  ...
  separate document. A literal that appears mid-chain with no owning layer is an _orphan_ — and
  the [custom ESLint rules](../.claude/skills/eslint-custom-rules/SKILL.md) and the
...
- **Coverage report / audit** — both derive from existing records automatically, so neither needs
  ...
  (`npm run report:coverage`); in RM it is generated by the platform.
...
- **CES** has nothing to drift _to_: a fact exists once and is imported, so a change is applied
...
- **CES** extracts the report _from the tests themselves_ — titles form the Given/When/Then and
  `req` metadata carries priority, story refs, and bugs. `npm run report:coverage` /
  `coverage:report:<type>` / `coverage:check` derive an always-current view because the source and the
  report are the same artefact.
- **RM** authors the report _in the platform_, from records that a human keeps aligned with the
