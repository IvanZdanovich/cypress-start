  ...
  function walkDirectory(dir) {
    try {
      ...
      for (const entry of entries) {
        ...
        } else if (entry.isFile() && /\.(js|cy\.js)$/.test(entry.name) && !entry.name.includes('test-data')) {
          files.push(fullPath);
        }
      }
    } catch {
...
/**
 * Extract all test-data instances from test-data files
...
/**
 * Search for test-data usage in test files.
 ...
 *     module scope (buildFileIndex), so repeated linting of multiple test-data
...
/**
 * Check for unused test-data instances
 */
...
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Find unused test-data instances from test-data files',
  ...
  create(context) {
    ...
    // Only run on files that live inside a test-data/ directory.
    // Using a path-separator-aware check prevents false matches on filenames
    // that merely contain the substring "test-data" (e.g. find-unused-test-data.js).
