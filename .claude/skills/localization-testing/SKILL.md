...
description: Use when asserting localized UI text or maintaining flat `l10n` keys across locale JSON, generated types, Cypress specs, and commands.
---

# Reasoning Principles

CONTRACT: the flat key string is the shared contract between frontend and tests, and this skill sets its grammar and reuse — otherwise the two code bases drift with no single place to join them again
FLATNESS: keys stay flat dot-namespaced strings, never nested objects — otherwise nested keys break translation lookup and are missed by the type union
...
NORMALIZATION: inconsistent source spellings resolve to one form per concept — otherwise mixed spellings split a single concept across keys and defeat reuse
...
TYPED_UNION: `cypress/support/l10n.d.ts` types `l10n` as `Record<L10nKey, string>` and is generated from `en-localization.json`; CLI validate/sync catches per-language drift
...
LANGUAGE_SWITCH: a spec asserts text after changing language → run with the target `LANGUAGE`, assert `l10n['key']` so the expected text comes from the active language, never a literal translation

## Key Grammar

KEY_SHAPE: `feature.area.element[.role]`, `lowerCamelCase` segments, dot-joined
KEY_DEPTH: 3-4 segments preferred, 5 the ceiling for table headers and option groups
...
REGEN: run `npm run l10n:gen-types` (also runs in `pretest`) to refresh `cypress/support/l10n.d.ts` from the reference `en-localization.json` — the `L10nKey` union that gives autocomplete and turns a bad key into a type error
USAGES: specs and commands read strings as `l10n['<key>']`; list related keys with a prefix filter, never with a `t()`/`tGroup()` wrapper or nested `l10n.<path>` access
...
LANGUAGE_SELECTION: the `LANGUAGE` env var drives the pretest copy of the selected locale file into the active map, so specs asserting `l10n['key']` work for any active language
LOOKUP_SHAPE: specs and commands use literal bracket access `l10n['itemList.title']`; prefix scans use `Object.keys(l10n).filter((key) => key.startsWith('common.status.'))`
...
NORMALIZATION_CHECK: one spelling per concept, placeholder defects logged in `bug-log/`
