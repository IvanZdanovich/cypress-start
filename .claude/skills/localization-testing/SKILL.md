...
CONTRACT: the flat key string is the shared contract between frontend and tests, and this skill governs its grammar and reuse — otherwise the two code bases drift with no single authority to reconcile them
...
TYPED_UNION: `cypress/support/l10n.d.ts` types `l10n` as `Record<L10nKey, string>`, generated by REGEN from the reference `en-localization.json` so the union is the complete, language-independent key set — per-language drift is caught by CLI_VALIDATE / CLI_SYNC, not by shrinking the union
